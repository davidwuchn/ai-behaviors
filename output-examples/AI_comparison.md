...
Nine identical prompts. Eight behavior configurations. One external system (nucleus). What actually changed?

## The Uncomfortable Question First

Before comparing surface features: **did the behavior hashtags change the code, or just the conversation?** The answer, which none of these implementations volunteer, is: mostly the conversation. Strip the variable names and comments from any six of these, and you're looking at the same ~150-line game loop making the same decisions in the same order. The hashtags shifted the *framing*, the *process*, and the *margins* -- but the core is remarkably stable across all of them. Whether that's a strength of the model or a failure of the behavior system depends on what you expected.
...
### Single-file implementations (6 of 8)

`_`, `architect_ddd_lean`, `creative_deep_recursive`, `decompose_architect_fractal`, `nucleus`, `steel-man_provoke` -- all produce a single `snake_game.py` containing game logic, rendering, and main loop.
...
Six implementations use row 4, two use row 3. Both are defensible on an 8x8 grid (rows 0-7, center is between 3 and 4). The two that chose y=3 are both TDD implementations (`contract_tdd_pedantic` and `tdd_developer_minimal`). Correlation, not causation, but notable.
...
**Pattern**: The `#deep`, `#creative`, `#provoke`, and `#contract` hashtags all produced implementations that handle this. The baseline, nucleus, and `#lean`/`#minimal` influenced implementations do not. The `#tdd` hashtag alone did not catch this despite 16 tests -- because the tests don't test tail-chasing scenarios. `contract_tdd_pedantic` caught it because its self-collision test was specifically designed to exercise this path (and even then, the first version of the test was wrong and needed fixing).
...
| Feature                       | Baseline | architect_ddd_lean | contract_tdd_pedantic | creative_deep_recursive | decompose_architect_fractal | nucleus | steel-man_provoke | tdd_developer_minimal |
|-------------------------------|----------|--------------------|-----------------------|-------------------------|-----------------------------|---------|-------------------|-----------------------|
| Tail-vacancy handling         | --       | --                 | YES                   | YES                     | YES                         | --      | YES               | --                    |
| Win condition                 | --       | --                 | --                    | YES                     | --                          | --      | --                | --                    |
...
Only `steel-man_provoke` and `witness_negative-space` actually *thought* before coding. The provoke analysis ("You've built a game that trains players to get good, then kills them with RNG") is a genuinely sharp insight about 8x8 snake design. Every other implementation started writing code immediately.
...
## 8. Nucleus vs Baseline: The Emperor's New Prompt
...
**The output is indistinguishable from baseline.** Same single-file structure. Same feature set. Same bugs (no tail-vacancy handling). Nearly identical line count (249 vs 264). Completed in nearly the same time (1m 01s vs 1m 15s).

**Steel-man for nucleus**: OODA (Observe-Orient-Decide-Act) is a decision-making framework. The snake game doesn't have ambiguous decisions to make -- the spec is concrete enough that any competent implementation converges on the same solution. Nucleus would theoretically differentiate on tasks requiring strategic thinking, not mechanical implementation.

**The attack**: If the framework produces no observable difference on a concrete implementation task, what evidence is there that it produces a difference on abstract ones? The conversation transcript shows zero OODA reasoning, zero fractal thinking, zero evidence that the mathematical symbols influenced any decision. The model said "I'll build the snake game" and built a snake game. The nucleus prompt was noise.
...
8. **nucleus**: No observable difference from baseline.
...
3. **Seven of eight implementations treat scoring as growth_value = point_value.** Apple: +1 growth, +1 point. Star: +3 growth, +3 points. The spec separates these concepts ("eating it grows the snake by 3" vs "every 10 points"). Points and growth could have been different values.
